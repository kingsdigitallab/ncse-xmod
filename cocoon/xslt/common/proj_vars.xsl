...
<!-- $Id: proj_vars.xsl 732 2008-02-05 11:53:56Z zau $ -->

<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:saxon="http://icl.com/saxon"
  xmlns="http://www.w3.org/1999/xhtml" extension-element-prefixes="saxon" version="1.1"
  xmlns:filename="filename.uri" xmlns:ex="ex.uri" xmlns:vp="vp.uri" xmlns:tm="tm.uri"
  xmlns:pr="pr.uri" exclude-result-prefixes="saxon filename ex vp tm pr">
  ...
  <xsl:variable name="group-type02-path"
    select="//filebase//group[@id=$group-type02-id]/groupHead/groupFolder"/>
  <xsl:variable name="group-type03-path"
    select="//filebase//group[@id=$group-type03-id]/groupHead/groupFolder"/>
  ...
  <xsl:variable name="dash"
    select="'ABCDEFGHIJKLMNOPQRSTUVWXYZ-abcdefghijklmnopqrstuvwxyz_0123456789'"/>
  ...
  <xsl:template name="tpl_rhcontent">
    ...
    <xsl:variable name="rhc-type02-val"
      select="//filebase//group[@id=$group-type02-id]/groupHead/rhcontent/@value"/>
    ...
    <xsl:variable name="rhc-type03-val"
      select="//filebase//group[@id=$group-type03-id]/groupHead/rhcontent/@value"/>
    ...
    <xsl:variable name="rhc-group-val"
      select="//filebase//group[item[@id=current()/@id]]/groupHead/rhcontent/@value"/>
    ...
    <xsl:choose>
      ...
      <xsl:when
        test="string(@rend) and (contains(normalize-space(@rend), 'on') or contains(normalize-space(@rend), 'off'))">
        <xsl:choose>
  ...
  <xsl:template name="tpl_theme">
    ...
    <xsl:variable name="th-type02-val"
      select="//filebase//group[@id=$group-type02-id]/groupHead/theme/@value"/>
    ...
    <xsl:variable name="th-type03-val"
      select="//filebase//group[@id=$group-type03-id]/groupHead/theme/@value"/>
    ...
    <xsl:variable name="th-group-val"
      select="//filebase//group[item[@id=current()/@id]]/groupHead/theme/@value"/>
