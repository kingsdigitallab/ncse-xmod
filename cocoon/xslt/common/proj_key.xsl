...
<xsl:stylesheet exclude-result-prefixes="#all" version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
  ...
  <xsl:template name="ctpl_nav">
    ...
    <xsl:choose>
          ...
          <xsl:if test="descendant::*[@ref=$context-id] or starts-with($context-id, .//file/@start) and string(.//file/@start)">
            <xsl:choose>
              <xsl:when test=".//file[@ref=$context-id or starts-with($context-id, @start) and string(@start)]">
                ...
                <ul class="pn2">
                  <!-- Link for the first level page -->
                  <!-- NB - uses fileTitle in fileBase for text so that it is not the same as the heading -->
                  <li class="s01">
                    <a>
                      <!-- Current page Class -->
                      <xsl:if test="default/@ref=$loc-id">
                        <xsl:attribute name="class">
      ...
      <!-- START default navbar items -->
      <xsl:otherwise>
        <!--  START level 1 navigation  -->
        <xsl:for-each select="//navbar//body/layout/*">
