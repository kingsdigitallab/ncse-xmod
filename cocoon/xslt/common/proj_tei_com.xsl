<?xml version="1.0"?>
<!--  $Id: proj_tei_com.xsl 732 2008-02-05 11:53:56Z zau $ -->
...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:saxon="http://icl.com/saxon"
  ...
  <xsl:template
    match="/aggregation/TEI.2/teiHeader/fileDesc/titleStmt/*[self::author|self::editor|self::respStmt]"
    mode="pagehead">
    <xsl:apply-templates mode="pagehead"/>
  ...
  <xsl:template match="/aggregation/TEI.2/teiHeader/fileDesc/titleStmt/respStmt/resp"
    mode="pagehead">
    <xsl:apply-templates/>
...
<xsl:template name="ctpl_pagehead">
 ...
 -->
    ...
    <xsl:choose>
      <xsl:when test="../@type='gloss'">
        <xsl:variable name="count-item">
          <xsl:number level="single" count="item" format="01"/>
        </xsl:variable>
        <xsl:choose>
          <xsl:when test="preceding-sibling::headLabel or preceding-sibling::headItem">
            <xsl:text>r</xsl:text>
            <xsl:number value="$count-item + 1" format="01"/>
          </xsl:when>
          <xsl:otherwise>
            <xsl:text>r</xsl:text>
  ...
  <xsl:template name="showFigure">
    ...
    <xsl:variable name="img-max-height-full">
      <xsl:for-each select="ancestor::list[starts-with(@type, 'figure')]/item">
        ...
        <xsl:variable name="cur-img-height"
          select="number(document('../../xml/03_alist/AL_imageBase.xml')//image[@id=$cur-img-url]/height)"/>
        <xsl:sort
    ...
    <xsl:variable name="img-max-height-thumb">
      <xsl:for-each
        select="ancestor::list[not(@type='figure-list')][starts-with(@type, 'figure')]/item">
        <xsl:variable name="cur-img-url" select="figure/@url"/>
        <xsl:variable name="cur-img-height"
          select="number(document('../../xml/03_alist/AL_imageBase.xml')//image[@id=$cur-img-url]/thmHeight)"/>
        ...
        <xsl:choose>
          <xsl:when test="@type='thumb'">
            <a href="{$img-path-full}">
              ...
              <span>&#160;</span>
              <img src="{$img-path-thumb}" class="s{$img-left-right}">
                <!-- @alt info -->
                <xsl:if test="string($img-cap-desc)">
                  <xsl:attribute name="alt">
                    <xsl:value-of select="$img-cap-desc"/>
          ...
          <xsl:otherwise>
            <img src="{$img-path-full}" class="s{$img-left-right}">
              <!-- @alt info -->
  ...
  <xsl:template match="xref">
    ...
    <xsl:variable name="internal-link-group-path-2-prep"
      select="substring-before(substring-after(@url, $text-type03-pre), '_')"/>
    ...
    <!-- Other variables -->
    <xsl:variable name="internal-link-group-filename"
      select="substring-after(@url, $text-type03-pre)"/>
