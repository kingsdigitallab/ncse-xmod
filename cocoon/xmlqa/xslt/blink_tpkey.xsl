<?xml version="1.0"?>
<!-- $Id: blink_tpkey.xsl 598 2007-09-03 15:51:05Z zau $ -->
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:saxon="http://icl.com/saxon"
  ...
  <xsl:template name="ctpl_content">
    ...
    <table width="100%" cellpadding="6" class="detail">
      ...
      <xsl:for-each select="//source//xref[@type='internal' or @rend='internal']">
        <xsl:variable name="cur-id" select="ancestor::dir:xpath/text()[1]"/>
        <xsl:if
          test="not(//dir:file/dir:xpath/text()[1] = current()/@from) or not(/aggregation/filebase//item[@id = current()/@from])">
          <tr>
            <td valign="top">
              <xsl:choose>
            ...
            <td valign="top">
              <xsl:if test="not(//dir:file/dir:xpath/text()[1] = current()/@from)">
                <xsl:attribute name="class">warning</xsl:attribute>
