...
def getArticle(con, faidid, faid, format, fdoit, fbadocr):
    # arttext = ""
...
if __name__ == '__main__':
    ...
    parser.add_option("-f", "--format", dest="outformat",
                      type="choice", choices=["TXT", "TXTHDR", "TXTSEMTAG", "XMLFAID", "XMLFTOKENID", "XMLSTOKENID", "LUCFAID", "CLUTO", "TMSK", "HELP", "help", "?"], default="HELP",
                      help="format of output files: 'TXT', 'TXTHDR', 'TXTSEMTAG', 'XMLFAID', 'XMLFTOKENID', 'XMLSTOKENID', 'LUCFAID', 'CLUTO', 'TMSK') - type '-f HELP' or '-f ?' for an explanation of the options - default: %default", metavar="FORMAT")
    ...
    for (id, fullartid, tokencount, charcount, charnscount, linecount) in fullartidslist:
        articletext = getArticle(dbcon, id, fullartid, outformat, filterdoit, filterbadocr)
