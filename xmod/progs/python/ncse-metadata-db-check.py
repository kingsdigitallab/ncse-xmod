...
if __name__ == '__main__':
    ...
    try:
        ...
        sys.exit(2)
    logpath = "./logs"
    repf = getRepfileObject(logpath, articlepath, "ncse-metadata-db-check")

    print "Reading fullartids from 04 version ..."
    faid04list = getFullArtIds(md04con, articlepath)
    PrintCF(repf, 1, "-" * 50)
    PrintCF(repf, 1, "%s: total no. of fullartids in 04: %i" % (articlepath, len(faid04list)))

    print "Reading fullartids from 05 version ..."
    faid05list = getFullArtIds(md05con, articlepath)
    PrintCF(repf, 1, "-" * 50)
    PrintCF(repf, 1, "%s: total no. of fullartids in 05: %i" % (articlepath, len(faid05list)))
    ...
    print "Looking up fullartids from 05 in 04 ..."
    faidnotin05list = [f for f in faid04list if f not in faid05list]
    print "Looking up fullartids from 04 in 05 ..."
    ...
    PrintCF(repf, 1, "")
    PrintCF(repf, 1, "-" * 50)
    PrintCF(repf, 1, "%s: fullartids in 04, but not in 05" % (articlepath, ))
    PrintCF(repf, 1, "-" * 50)
    PrintCF(repf, 1, "")
    PrintCF(repf, 1, "     Number of fullartids not in 05: %i" % (len(faidnotin05list), ))
    PrintCF(repf, 1, "")
    PrintCF(repf, 1, "     List of fullartids not in 05:")
    ...
    PrintCF(repf, 1, "-" * 50)
    PrintCF(repf, 1, "%s: fullartids in 05, but not in 04" % (articlepath, ))
    PrintCF(repf, 1, "-" * 50)
    PrintCF(repf, 1, "")
    PrintCF(repf, 1, "     Number of fullartids not in 04: %i" % (len(faidnotin04list), ))
    PrintCF(repf, 1, "")
    PrintCF(repf, 1, "     List of fullartids not in 04:")
