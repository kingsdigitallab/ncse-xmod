...
if __name__ == '__main__':
    ...
    for faidin in faidinlist:
        ...
        for f in rl:
            ...
            #
            srcprofileset = set(srcprofiledic.keys())
            targetprofileset = set(targetprofiledic.keys())
            profileintersect = srcprofileset & targetprofileset
            maxoutofplace = len(profileintersect)
            # print srcprofileset
            # print targetprofileset
            # print maxoutofplace, profileintersect
            outofplace = 0
            for semtag in srcprofiledic.keys():
                if semtag in profileintersect:
                    # deduct on llh from the other
                    #    the smaller the result the more similar are the two llh values
                    #    but this is maybe not reliable as it doesn't take into account
                    #    that a high ranking value in one set can have a low llh value
                    #    whereas the corresponding value in the other set has a high one
                    # maybe we should try to construct a new ranking just of the
                    #    intersected values and calculate the out-of-place value from
                    #    these
            ...
            if outofplace < useoutofplace:
                print faidout, maxoutofplace, outofplace, outofplace/maxoutofplace
                # print srcprofiledic
                # print targetprofiledic
