...
# Time-stamp: <Thu 18.09.2008 10:59:10 BST gb>

# Variant of prxml-to-tokenDB.py that only writes to fullartids table
#                                           (for performance reasons)
# tokens table is written as TAB delimited files that can be 
#     that can be loaded into the DB 
# 
# TODO: fix character encoding
#       check byarttext.py --- t = t.encode("utf-8") in normaliseHyphen
#                              try not using unicodedata for emdash and endash
#       DON'T FORGET to change XnormaliseHyphen back to its original name
#       DON'T FORGET to revert the temporary changes in getWApfsVal
# 
...
#       dirs in list
# DONE: ZIP files can also be called Documents.zip
# DONE: publ, year, month, day --> fullartid
# DONE: add token count and Olive word count to fullartid
# DONE: add charcount, charnscount, linecount to fullartid
# DONE: fix case insensitivity in types and lctypes
...
def createLoadDataInfileShellScript():
    (p, loaddatashellscriptname) = os.path.split(confvarsdic["corpusinpath"])
    loaddatashellscriptname += ".sh"
    loaddatashellscriptpath = os.path.join(confvarsdic["db_load_path"], loaddatashellscriptname)
    datapath = os.path.join(confvarsdic["db_mysql_data_path"], confvarsdic["db_name"])
    shscriptobj = file(loaddatashellscriptpath, "w")
    print >> shscriptobj, "#!/bin/sh"
    print >> shscriptobj, 'echo "########## FLUSH TABLES"'
    print >> shscriptobj, "mysql -u gbrey -e 'FLUSH TABLES' %s" % (confvarsdic["db_name"] ,)
    print >> shscriptobj, 'echo "########## TURN OFF INDEXING"'
    print >> shscriptobj, "sudo myisamchk --keys-used=0 -rq %s/tokens.MYI" % (datapath ,)
    for outfilepath in outfilepathlist:
        (d, of) = os.path.split(outfilepath)
        (f, e) = os.path.splitext(of)
        print >> shscriptobj, 'echo "########## LOAD: %s"' % (f, )
        print >> shscriptobj, "mysql -u gbrey -e 'LOAD DATA INFILE \"%s\" INTO TABLE tokens' %s" % (outfilepath ,confvarsdic["db_name"])
    print >> shscriptobj, 'echo "########## TURN ON INDEXING"'
    print >> shscriptobj, "sudo myisamchk -rq %s/tokens.MYI" % (datapath ,)
    print >> shscriptobj, 'echo "########## FLUSH TABLES"'
    print >> shscriptobj, "mysql -u gbrey -e 'FLUSH TABLES' %s" % (confvarsdic["db_name"] ,)
    print >> shscriptobj, 'echo "########## FINISHED"'
    shscriptobj.close()
            ...
            if len(walkfiles) > 0:
                if confobj.confvarsdic["db_output"] == "File":
                    ...
                    if curroutfilename != outfilename:
                        ...
                        outfilepath = os.path.join(confobj.confvarsdic["db_load_path"], curroutfilename)
                        outfilepathlist.append(outfilepath)
                ...
                for artid in artrefslist:
                    ...
                    for tokenl in newart.tokenlist:
                        ...
                        # 
                        # writing to types and lctypes table is now done separately
                        # 
                        ...
                        if confobj.confvarsdic["db_type"] == "SQLite":
                            ...
                            ###### 01.02.2008: tokensfieldvaldic["token"] = token
                            # if chr(166) in token:
                            #     print "#############", token
                            #     sys.exit()
                            # print "#################", tokenid, type(token), token
                            # print "#################", token
...
#                                                   ]
# --------------------------------------
                             # in a TAB separated list we don't need to
                             # enclose string within quotation marks
                             # first field is id - set to 0
                             fvlist.append("0") 
                             for (tfn, s) in tokensfieldnamelist:
                                 # fnlist.append(tfn)
                                 # if tfn == "token":
                                 #     print type(tokensfieldvaldic[tfn])
                                 #     print type(str(tokensfieldvaldic[tfn]))
                                 # fvlist.append(str(tokensfieldvaldic[tfn]))
                                 fvlist.append(str(tokensfieldvaldic[tfn]))
                             # print fnlist
                             # print fvlist
                             # print "-" * 40
...
# --------------------------------------
    ...
    if confobj.confvarsdic["db_output"] == "File":
        outfileobj.close
        createLoadDataInfileShellScript()
