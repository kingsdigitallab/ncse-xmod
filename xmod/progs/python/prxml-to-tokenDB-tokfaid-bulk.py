...
# Variant of prxml-to-tokenDB.py that only writes to tokens table
#         and fullartids table (for performance reasons)
                ...
                for artid in artrefslist:
                    ...
                    for tokenl in newart.tokenlist:
                        ...
                        if confobj.confvarsdic["db_output"] == "DB":
                            ...
                            outfileobj.flush()
                    # 
                    # bulk write tokens for on article
                    # 
                    # print bulkfvlist
                    # We have to test if it is an article with text in it
                    # and not just an empty "Pc" article
                    if bulkfvlist != []:
                        insobj = SqlBulkInsert(con=dbcon, 
                                           table="tokens", 
                                           fieldnames=fnlist, 
                                           bulkfieldvalues=bulkfvlist)
                        tid = insobj.getLastRowId()
                        del insobj
                    del newart
